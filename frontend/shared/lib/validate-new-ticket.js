...
export const EDIT_TICKET_FIELD_IDS = {
  title: 'et',
  description: 'ed',
};
...
/** @param {{ title?: string, description?: string }} draft */
export function validateTicketEditDraft(draft) {
  const errors = [];
  ...
  return {
    valid: errors.length === 0,
    errors,
    firstFieldId: first ? EDIT_TICKET_FIELD_IDS[first.field] : null,
    summaryItems: errors.map((e) => e.summary),
    liveMessage: errors.map((e) => `${e.label}: ${e.message}`).join(' '),
  };
}
