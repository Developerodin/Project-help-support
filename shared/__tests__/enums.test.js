...
import {
  ROLE_IDS, ROLES, INTERNAL_ROLES, EXTERNAL_ROLES, PEOPLE_ASSIGNABLE_ROLES, ROLE_LABELS,
...
const ALL = {
  ROLES, INTERNAL_ROLES, EXTERNAL_ROLES, PEOPLE_ASSIGNABLE_ROLES,
  ADMIN_ROLES, IMPERSONATION_INITIATOR_ROLES, PROJECT_ADMIN_ROLES,
  SEVERITIES, PRIORITIES, CATEGORIES, LABELS, ENVIRONMENTS, LINK_RELS, STAGE_DECISIONS,
};
...
test('PEOPLE_ASSIGNABLE_ROLES includes client roles but excludes super_admin and read_only', () => {
  ...
  assert.ok(PEOPLE_ASSIGNABLE_ROLES.includes(ROLE_IDS.DEVELOPER));
  assert.ok(!PEOPLE_ASSIGNABLE_ROLES.includes(ROLE_IDS.SUPER_ADMIN));
  assert.ok(!PEOPLE_ASSIGNABLE_ROLES.includes(ROLE_IDS.READ_ONLY));
});
